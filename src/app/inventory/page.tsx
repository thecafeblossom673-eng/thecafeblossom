...
import { Bill, Expense, DayClose } from '@/lib/supabase';
import { 
...
export default function InventoryExpensesPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [billHistory, expenseList, dayCloseHistory, tablesList, todayClosedCheck] = await Promise.all([
        db.getBillHistory(),
        db.getExpenses(),
        db.getDayCloses(),
        db.getTables(),
        db.isDayClosed(todayDate)
      ]);

      setBills(billHistory);
      setExpenses(expenseList);
      setDayCloses(dayCloseHistory);
      setIsTodayClosed(todayClosedCheck);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      await db.addExpense(category, Number(amount), description.trim());
      setDescription('');
      setAmount('');
  ...
  const handleCloseDay = async (e: React.FormEvent) => {
    ...
    if (occupiedTables.length > 0) {
      alert('Cannot close the day while tables are occupied. Please close all active table orders first.');
      return;
    }

    if (!confirm("Are you sure you want to close today's sales? This will log today's totals permanently.")) {
      return;
    }
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="bg-primary text-primary-foreground sticky top-0 z-40 shadow-md">
        <div className="mx-auto max-w-5xl px-4 py-3 flex items-center justify-between">
          ...
          <button onClick={fetchData} className="text-xs text-primary-foreground underline cursor-pointer font-sans">
            Refresh
          </button>
        </div>
      </header>
                  ...
                  <form onSubmit={handleCloseDay} className="space-y-4 font-sans text-sm">
                    ...
                    <div className="bg-muted/40 border border-border/75 rounded-xl p-4 space-y-2.5">
                      ...
                      <div className="border-t border-border/50 pt-2 flex justify-between text-sm font-bold">
                        ...
                        <span className={`font-mono ${todayProfit >= 0 ? 'text-green-700' : 'text-red-700'}`}>
                    ...
                    <div>
                      <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5">
                        Day-End Closure Notes
                      </label>
                      <textarea
                        value={dayCloseNotes}
                        onChange={e => setDayCloseNotes(e.target.value)}
                        placeholder="e.g. Cash drawer matches sales, smooth day, shift closed by staff members"
                        rows={3}
                        className="w-full px-3 py-2 bg-background text-foreground border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary"
                      />
                    </div>
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 rounded-xl bg-primary text-primary-foreground px-4 py-3 text-sm font-bold uppercase tracking-wider hover:bg-primary/90 disabled:opacity-50 transition-all cursor-pointer shadow-sm"
                    >
                      {busy ? (
                        <Loader2 className="h-4 w-4 animate-spin" />
                      ) : (
